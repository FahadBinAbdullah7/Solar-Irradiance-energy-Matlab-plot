...
for month = 1:12
    ...
    for day = 1:month_days(month)
        ...
        for i = 1:p
            ...
            %  calculations for the current time of the day
        ...
        % Add daily irradiance to the sum for the current month
        total_monthly_irradiance = total_monthly_irradiance + sum(total);
        
        day_counter = day_counter + 1;
    end

    % Calculate the total energy for the current month (Day 15 value * total days in month)
    monthly_energy(month) = total_irradiance(day_counter - month_days(month)) * month_days(month);
end


months_ordered = categorical(months, months);

% Plotting the total energy for each month as a bar chart
figure;
bar(months_ordered, monthly_energy);
xlabel('Months');
ylabel('Total Energy (Watt/m^2)');
title('Total Energy per Month');
